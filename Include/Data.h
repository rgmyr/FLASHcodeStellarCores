...
struct Data 
{
	...
	{	
		if (id >= totalSize) {
			std::cout << "Error: trying to convert the index of a saddle into xyz" << std::endl;
			x = y = z = 0xffffffff;
			return;
		}
	...
	void loadFromVector(const std::vector<float>& gpot, const std::vector<float>::size_type totalSize); 
